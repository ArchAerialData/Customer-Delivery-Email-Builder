...
MEDIA_SUBJECT = {
    ...
    "photos, and 2D mapping": "Photos and 2D Mapping",
...
class EmailTemplateApp:
    ...
    def _load_data(self):
        self.master_data = self._read_master_data()
        self.dropbox_data = [
    ...
    def _load_signature_html(self, signature_name=None):
        sig_dir = Path.home() / "AppData" / "Roaming" / "Microsoft" / "Signatures"
        target = (signature_name or "").strip()
        candidates = []
        if target:
        ...
        if not candidates:
            if target:
                ...
                candidates = sorted(sig_dir.glob("*.htm"))
    ...
    def _rehydrate_template_image_sources(self, html, signature_name=None):
        ...
        signature_html = path.read_text(encoding="utf-8", errors="ignore")
        signature_sources = [
            match.group(2)
            for match in re.finditer(r'src=(["\'])([^"\']+)\1', signature_html, flags=re.IGNORECASE)
            if not match.group(2).lower().startswith(("cid:", "http"))
        ]
        ...
        def _replace(match):
            nonlocal idx
            if idx >= len(signature_sources):
                return match.group(0)
            quote = match.group(1)
            replacement = signature_sources[idx]
            idx += 1
            return f"src={quote}{replacement}{quote}"

        return re.sub(r'src=(["\'])cid:[^"\']+\1', _replace, html, flags=re.IGNORECASE)
    ...
    def _build_ui(self):
        ...
        notebook.add(self.master_tab, text="Client Sites")
    ...
    def _build_compose_tab(self):
        ...
        ttk.Label(left, text="Body").pack(anchor="w")
        ...
        options_row = ttk.Frame(left)
        options_row.pack(anchor="w", fill="x", pady=(0, 2))

        ttk.Checkbutton(options_row, text="Show BCC", variable=self.show_bcc_var, command=self._toggle_bcc).pack(side="left")

        self.template_folder_frame = ttk.Frame(options_row)
        self.template_folder_frame.pack(side="left", padx=(18, 0))
        ...
        ttk.Button(left, text="Create Outlook Draft", command=self._create_draft).pack(anchor="w", pady=(6, 6))
    ...
    def _build_master_tab(self):
        frame = ttk.Frame(self.master_tab)
        ...
        self.master_tree.heading("client", text="Client")
        self.master_tree.heading("site", text="Site")
        self.master_tree.heading("email", text="Operations Contact Emails")
        self.master_tree.heading("link", text="Customer Dropbox URL")
        self.master_tree.column("client", width=180)
        self.master_tree.column("site", width=240)
        self.master_tree.column("email", width=320)
        self.master_tree.column("link", width=420)
        master_scroll = ttk.Scrollbar(
            tree_frame,
            orient="vertical",
            command=self.master_tree.yview,
            style="ClientSites.Vertical.TScrollbar",
        )
        self.master_tree.configure(yscrollcommand=master_scroll.set)
        tree_frame.rowconfigure(0, weight=1)
        tree_frame.columnconfigure(0, weight=1)
        self.master_tree.grid(row=0, column=0, sticky="nsew")
        master_scroll.grid(row=0, column=1, sticky="ns")
        self.master_tree.bind("<<TreeviewSelect>>", lambda _e: self._load_master_selection())
        ...
        ttk.Label(form, text="Operations Contact Emails").grid(row=0, column=2, sticky="w")
        ttk.Label(form, text="Customer Dropbox URL").grid(row=0, column=3, sticky="w")

        self.master_client_var = tk.StringVar()
        self.master_site_var = tk.StringVar()
        self.master_email_var = tk.StringVar()
        self.master_link_var = tk.StringVar()

        ttk.Entry(form, textvariable=self.master_client_var, width=26).grid(row=1, column=0, sticky="w", padx=(0, 10))
        ttk.Entry(form, textvariable=self.master_site_var, width=32).grid(row=1, column=1, sticky="w", padx=(0, 10))
        ttk.Entry(form, textvariable=self.master_email_var, width=40).grid(row=1, column=2, sticky="w", padx=(0, 10))
        ttk.Entry(form, textvariable=self.master_link_var, width=52).grid(row=1, column=3, sticky="w")

        btns = ttk.Frame(frame)
        btns.pack(fill="x", pady=4)
        ttk.Button(btns, text="Add", command=self._add_master_entry).pack(side="left", padx=(0, 6))
        ttk.Button(btns, text="Update", command=self._update_master_entry).pack(side="left", padx=(0, 6))
        ttk.Button(btns, text="Delete", command=self._delete_master_entry).pack(side="left", padx=(0, 6))
        ttk.Button(btns, text="Reload", command=self._reload_master_tab).pack(side="left", padx=(0, 6))
    ...
    def _trim_leading_main_block(self, template_name: str, html: str) -> str:
        if not html:
            return html
        marker = "Our pilots captured progress {media_body} of the {site} site on {date}."
    ...
    def _reload_compose_tab(self):
        ...
        self._reload_master_data(refresh_clients=True, refresh_master_tree=False)
        self._refresh_cc_lists()
        self._reset_compose_state(refresh_clients=True, force_blank_client_site=True)

    def _reload_master_tab(self):
        # Reload only Client Sites tab data
        self._reload_master_data(refresh_clients=False, refresh_master_tree=True)
    ...
    def _on_site_change(self):
        ...
        if site:
            ...
            client_dropbox_rows = _client_rows(self.dropbox_data)
        ...
        if site:
            ...
            client_contact_rows = _client_rows(self.contacts_data)
    ...
    def _reset_master_form(self):
        if not hasattr(self, "master_client_var"):
            return
        self.master_client_var.set("")
        self.master_site_var.set("")
        self.master_email_var.set("")
        self.master_link_var.set("")
    ...
    def _compose_subject(self):
        ...
        if ctx["site"] and ctx["media_subject"] and ctx["date"]:
            return f"{ctx['site']} - Aerial Progress {ctx['media_subject']} - {ctx['date']}"
        return ""
    ...
    def _compose_body(self):
        ...
        main = (
            f"Our pilots captured progress {ctx['media_body']} of the {ctx['site']} site on {ctx['date']}. "
    ...
    def _compose_body_html(self):
        ...
        if template and (template.get("html") or template.get("text")):
            ...
            if template.get("html"):
                ...
                rendered = self._render_template_text(rendered, ctx)
                rendered = self._apply_legacy_placeholders(rendered, ctx)
                rendered = self._rehydrate_template_image_sources(rendered, selected_name)
                if ctx["dropbox_link"]:
            ...
            if template.get("text"):
                ...
                return body_html, []
        signature_html, signature_assets = self._load_signature_html()
        ...
        body_html = (
            ...
            f"<p style=\"margin:0 0 8px 0;\">Our pilots captured progress {ctx['media_body']} of the {ctx['site']} site on {ctx['date']}. "
    ...
    def _update_email_preview(self):
        ...
        # Status bar (minimal, VS Code-style)
    ...
    def _create_draft_graph(self, subject: str, body_html: str, signature_assets):
        ...
        bcc_list = self._parse_recipients(self.bcc_var.get().strip())
    ...
    def _load_master_selection(self):
        selected = self.master_tree.selection()
        if not selected:
            return
        row_id = int(selected[0])
        row = next((r for r in self.master_data if r["row"] == row_id), None)
        if not row:
            return
        self.master_client_var.set(row["client"])
        self.master_site_var.set(row["site"])
        self.master_email_var.set(row["email"])
        self.master_link_var.set(row["link"])

    def _add_master_entry(self):
        client = self.master_client_var.get().strip()
        site = self.master_site_var.get().strip()
        email = self.master_email_var.get().strip()
        link = self.master_link_var.get().strip()
        if not client:
    ...
    def _reload_master_data(self, refresh_clients: bool = True, refresh_master_tree: bool = True):
        try:
            self._sort_master_sheet()
        except Exception:
            pass
