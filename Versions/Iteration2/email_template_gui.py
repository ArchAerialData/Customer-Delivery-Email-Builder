...
DROPBOX_FILE = DATA_DIR / "Dropbox URLs (December 2025).xlsx"
DROPBOX_SHEET = "DropboxViewLinks"
CONTACTS_FILE = DATA_DIR / "2025FullSDDL.xlsx"
CONTACTS_SHEET = "2025 Full SDDL"
...
class EmailTemplateApp:
    ...
    def _read_dropbox_data(self):
        wb = openpyxl.load_workbook(DROPBOX_FILE, data_only=True)
        ws = wb[DROPBOX_SHEET]
        rows = []
        for idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
            client = (row[0] or "").strip() if isinstance(row[0], str) else row[0]
            site = (row[1] or "").strip() if isinstance(row[1], str) else row[1]
            link = (row[3] or "").strip() if isinstance(row[3], str) else row[3]
            if client and site:
                rows.append({
                    "row": idx,
                    "client": client,
    ...
    def _read_contacts_data(self):
        wb = openpyxl.load_workbook(CONTACTS_FILE, data_only=True)
        ws = wb[CONTACTS_SHEET]
        rows = []
        for idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
            client = (row[0] or "").strip() if isinstance(row[0], str) else row[0]
            site = (row[1] or "").strip() if isinstance(row[1], str) else row[1]
            email = (row[3] or "").strip() if isinstance(row[3], str) else row[3]
            if client and site:
                rows.append({
                    "row": idx,
                    "client": client,
    ...
    def _create_draft(self):
        log_path = BASE_DIR / "outlook_draft_debug.log"
        def _log(msg):
            try:
                timestamp = _dt.datetime.now().isoformat()
                if not log_path.exists():
                    log_path.write_text(f"{timestamp}\t{msg}\n", encoding="utf-8")
                else:
    ...
    def _add_dropbox_entry(self):
        ...
        if not client or not site:
            ...
            return
        wb = openpyxl.load_workbook(DROPBOX_FILE)
        ws = wb[DROPBOX_SHEET]
        ws.append([client, site, None, link])
        wb.save(DROPBOX_FILE)
        wb.close()
    ...
    def _update_dropbox_entry(self):
        ...
        ws.cell(row=row_id, column=4, value=link)
        wb.save(DROPBOX_FILE)
        wb.close()
        self._reload_dropbox_data()
